...
def _field_page(c, y_top, num, note="заметки и эскизы в дороге"):
    """Разлинованная зона для рукописных заметок: линейки + уголок эскиза."""
    c.setStrokeColor("#9fb8c7"); c.setLineWidth(0.5)
    c.rect(34, 48, W - 68, y_top - 48, fill=0, stroke=1)
    c.setFillColor("#eef4fb")
    c.rect(34, 48, W - 68, y_top - 48, fill=1, stroke=0)
    c.setStrokeColor("#c3d2df"); c.setLineWidth(0.45)
    yy = y_top - 26
    while yy > 78:
        c.line(46, yy, W - 46, yy)
        yy -= 15
    c.setStrokeColor("#c3d2df"); c.setLineWidth(0.8)
    c.line(56, y_top - 26, 56, 48)
    c.setStrokeColor("#c3d2df"); c.setLineWidth(0.55)
    c.rect(W - 150, 56, 100, 78, fill=0, stroke=1)
    c.setFont("DejaVu-Obl", 7); c.setFillColor("#566573")
    c.drawString(W - 146, 126, "эскиз / рисунок")
    c.setFont("DejaVu-Bold", 8); c.setFillColor(DARK)
    c.drawString(40, y_top - 16, "■ " + note + "")
    c.setFont("DejaVu-Obl", 7.5); c.setFillColor("#566573")
    c.drawString(40, 34, "лист %02d/%02d · чистая страница для письма и рисунков" % (num, TOTAL))
...
def page_field(c, num, title, lines, concl=None, note="заметки и эскизы в дороге"):
    """Полевая страница: тезисы сверху, разлинованная зона для письма снизу."""
    _head(c, num, title, "полевая страница · %02d" % num)
    y = H - 74
    y = _text_block(c, 34, y, W - 68, lines, size=8.5)
    y -= 22
    _field_page(c, y, num, note=note)
    if concl:
        _concl(c, concl, 48)  # не перекрывает разлиновку
    _foot(c, num)
    c.showPage()
...
def page_risks(c, num):
    ...
    for name, lvl, desc in lines:
        ...
        y -= 46
    _field_page(c, y - 30, num, note="запиши собственные риски путешествия")
    _concl(c, "Наука честна: риск фиксируется до того, как модуль написан.", 48)
    _foot(c, num)
    c.showPage()
...
def build_pdf():
    ...
    page_field(c, 16, "11 · АЙС-ГЕНЕРАТОР — ЧТЕНИЕ В ИГРЕ", [
    ...
    ], concl="Набросай здесь схему загрузки профиля — как в дороге, карандашом.",
        note="полевая страница — проект маршрута загрузки айсберга")
    ...
    page_field(c, 23, "17 · ВЫВОДЫ", [
        ...
        note="дополни свои выводы в дороге")

    page_field(c, 24, "18 · ЗАКЛЮЧЕНИЕ", [
        ...
        note="заметки о будущем льда")

    page_field(c, 25, "ПРИЛОЖЕНИЕ А · СЛОВАРЬ", [
        ...
        note="впиши новые термины эпохи")
    ...
    page_field(c, 28, "ПРИЛОЖЕНИЕ Г · МЕРА ПАРТИИ", [
        ...
        note="место под собственный график партии")

    page_field(c, 29, "ПРИЁМКА · КОНТРОЛЬНЫЙ ЛИСТ", [
        ...
        note="оставь подпись проверяющего здесь")

    page_sign(c, 30)

    c.setTitle("Проектная работа — Ледяные человечки (детерминировано)")
    c.setAuthor("Ледяная Вечерка")
    c.setSubject("структура git, подходы, по науке")
    c.save()
    _fix_dates(PDF)
    return PDF
...
def _fix_dates(path):
    """reportlab пишет CreationDate и уникальный /ID → MD5 плавает.
    Заменяем метки времени и ID на фиксированные, чтобы два прогона давали один MD5."""
    import re as _re
    raw = open(path, "rb").read()
    raw = _re.sub(rb"\(D:\d{14}[^)]*\)", b"(D:20260101000000Z00'00')", raw)
    raw = _re.sub(rb"<\s*[0-9a-f]{32}\s*>\s*<\s*[0-9a-f]{32}\s*>",
                  b"<84c0ffee84c0ffee84c0ffee84c0ffee><84c0ffee84c0ffee84c0ffee84c0ffee>", raw)
