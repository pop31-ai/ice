# -*- coding: utf-8 -*-
"""Генератор polyart.html — «Полиарт-φ галерея»: все обложки в стиле
полиномиального искусства и золотого сечения + манифест и поводы для PR.
Числа (обложки, серии) вычисляются, чтобы статистика не старела."""
...
MANIFEST = [
    "1. У каждой обложки — своя кривая. Полином степени 3 задаёт слои неба — ни одна из %d не повторяет ни одну другую.",
    ...
    "4. Палитра серии — это характер издания: холодный свет Площади, огонь Шторма, золото Глянца и Золотого журнала. Цвет — мнение, а не украшение.",
    ...
    "8. Технологичность — тема PR: %d обложек сгенерированы кодом за минуты, но выглядят как ручная графика.",
...
<body>
<div class="wrap">
  <h1>✨ Полиарт-φ 🧊<small>%d обложек в стиле полиномиального искусства и золотого сечения — метод, а не шаблон</small></h1>
  <div class="top">
...
out = html
rows = "\n".join('<div class="mrow"><b>%s</b></div>' % (m % N_CO if "%d" in m else m)
                 for m in MANIFEST)
out = out.replace("@@MANIFEST@@", rows).replace("@@SERIES@@", "".join(blocks))
out = out.replace("%d обложек", "%d обложек" % N_CO)
open(os.path.join(BASE, "polyart.html"), "w", encoding="utf-8").write(out)
print("polyart.html written; covers:", N_CO)